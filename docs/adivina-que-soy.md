...
- Movimiento opcional, por permiso iniciado al tocar Activar. `cos(beta) * cos(gamma)` indica la normal de la pantalla respecto a la gravedad, tanto en vertical como en horizontal. Pantalla hacia abajo: acierto; hacia arriba: pasar. Exige 90 ms de posición vertical y 90 ms de inclinación sostenida. Cada acción reinicia el detector, pero el regreso al centro durante el aviso visual rearma la siguiente tarjeta. El umbral de acción es aproximadamente 27°; no hace falta inclinar hasta casi acostar la pantalla.
