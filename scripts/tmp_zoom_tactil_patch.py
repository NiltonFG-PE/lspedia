...
old_state = '''    inicioDesplazX: 0,
    inicioDesplazY: 0,
    listenersListos: false
};'''
new_state = '''    inicioDesplazX: 0,
    inicioDesplazY: 0,
    pellizcando: false,
...
if old_state not in txt:
    raise SystemExit('No se encontro el bloque de estado de zoom esperado')
txt = txt.replace(old_state, new_state, 1)

old_click = '''    // Clic simple: alterna entre 1x y 2.5x (comportamiento típico de "lightbox").
...
new_click = '''    // Clic simple: alterna entre 1x y 2.5x (comportamiento típico de "lightbox").
...
if old_click not in txt:
    raise SystemExit('No se encontro el listener click esperado')
txt = txt.replace(old_click, new_click, 1)

old_touch = '''    contenedor.addEventListener("touchstart", (ev) => {
...
new_touch = '''    // Gestos táctiles: pellizcar con dos dedos para ampliar/reducir y,
...
if old_touch not in txt:
    raise SystemExit('No se encontro el bloque tactil esperado')
txt = txt.replace(old_touch, new_touch, 1)
script.write_text(txt, encoding='utf-8')

css = Path('css/estilos.css')
c = css.read_text(encoding='utf-8')
old_css = '''.zoom-imagen-contenedor.arrastrando .zoom-imagen-img {
    transition: none; /* mientras se arrastra, que siga al dedo/mouse sin demora */
}'''
new_css = '''.zoom-imagen-contenedor.arrastrando .zoom-imagen-img,
.zoom-imagen-contenedor.pellizcando .zoom-imagen-img {
    transition: none; /* al arrastrar o pellizcar, la imagen sigue los dedos sin demora */
}'''
if old_css not in c:
    raise SystemExit('No se encontro el bloque CSS de zoom esperado')
c = c.replace(old_css, new_css, 1)
css.write_text(c, encoding='utf-8')

sw = Path('sw.js')
s = sw.read_text(encoding='utf-8')
s2, n = re.subn(r'const VERSION_APP = "v\d+";', 'const VERSION_APP = "v84";', s, count=1)
if n != 1:
